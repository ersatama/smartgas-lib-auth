<?php

namespace Ruslan_sgs\SmartgasLibAuth\Tests\Unit;

use Illuminate\Support\Facades\Hash;
use Orchestra\Testbench\TestCase;
use Ruslan_sgs\SmartgasLibAuth\Services\Auth\{
    AuthQueryService,
};
use PHPUnit\Framework\MockObject\Exception;
use Ruslan_sgs\SmartgasLibAuth\Helpers\SmscHelper;
use Ruslan_sgs\SmartgasLibAuth\Models\CrmUser;
use Illuminate\Support\Facades\Config;

class AuthTest extends TestCase
{
    protected AuthQueryService $authQueryService;

    /**
     * @throws Exception
     */
    public function setUp(): void
    {
        parent::setUp(); // TODO: Change the autogenerated stub
        $this->authQueryService = $this->createMock(AuthQueryService::class);
        $this->SmscHelper = new SmscHelper();
        Config::set('smartgas.smsc.url', 'https://smsc.kz/sys/send.php?');
        Config::set('smartgas.smsc.login', 'smartgas');
        Config::set('smartgas.smsc.password', 'SmartGas2020!');
    }

    /**
     * @dataProvider dataLogin
     */
    public function testLogin(array $userData)
    {
        $user = new CrmUser();
        $user->id = 1;
        $this->username = $userData['username'];
        $this->password = $userData['password'];
        $this->token = $userData['token'];
        $this->be($user);
        $this->authQueryService->method('login')->with([
            'username' => $userData['username'],
            'password' => $userData['password']
        ])->willReturn(
            $userData['token']
        );
        $token = $this->authQueryService->login([
            'username' => $userData['username'],
            'password' => $userData['password']
        ]);
        $this->assertEquals($token, $userData['token']);
    }

    public static function dataLogin(): array
    {
        $arr = [];
        for ($i = 1; $i < 10; $i++) {
            $arr[] = [
                [
                    'id' => 1,
                    'username' => 'test_1',
                    'password' => 1231231,
                    'token' => 'aasdasdasd'
                ]
            ];
        }
        return $arr;
    }

    /**
     * @dataProvider dataSendSmsc
     */
    public function testSendSmsc(int $phone, int $code)
    {
        $smsStatus = $this->SmscHelper->send($phone, $this->SmscHelper->phoneCodeVerify($code));
        $this->assertTrue($smsStatus);
    }

    public static function dataSendSmsc(): array
    {
        return [
            [77757253029, 140000]
        ];
    }
}